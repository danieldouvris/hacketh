...
contract('WCCToken', function(accounts) {
    var tokenInstance;

    it('allocates initial supply', function() {
        return WCCToken.deployed().then(function(instance) {
            tokenInstance = instance;
            return tokenInstance.name()
        }).then(function(name) {
            assert.equal(name, 'WCC Token', 'has the correct name')
            return tokenInstance.symbol();
        }).then(function(symbol) {
            assert.equal(symbol, 'WCC','has the correct symbol')
            return tokenInstance.standard();
        }).then(function(standard) {
            assert.equal(standard, 'WCC Token v.1', 'has the correct standard')
        })
    });
    ...
    it('handles delegated token transfers', function() {
        return WCCToken.deployed().then(function(instance) {
          ...
          // Transfer some tokens to fromAccpount
